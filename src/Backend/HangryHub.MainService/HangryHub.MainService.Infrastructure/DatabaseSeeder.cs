...
namespace HangryHub.MainService.Infrastructure
{
    public static class DatabaseSeeder
    {
        ...
        private static async Task SeedOrderForRestaurantAsync(MainDBContext dbContext, Restaurant restaurant, CustomerId userId)
        {
            ...
            var deliveryDataList = new List<DeliveryData>();

            for (int i = 0; i < Random.Shared.Next(3, 6); i++)
            {
                var deliveryGuid = Guid.NewGuid();

                deliveryDataList.Add(new(new(deliveryGuid))
                {
                    Contact = $"Test_Contact_{deliveryGuid}",
                    DeliveryLocation = $"Test_DeliveryLocation_{deliveryGuid}",
                    Note = $"Test_Note_{deliveryGuid}",
                    CustomerId = userId,
                });
            }

            dbContext.DeliveryDatas.AddRange(deliveryDataList);
            await dbContext.SaveChangesAsync();

            shoppingCart.Items = scItems;
            shoppingCart.SelectedDeliveryDataId = deliveryDataList
                .Skip(Random.Shared.Next(1, deliveryDataList.Count))
                .First().Id;
